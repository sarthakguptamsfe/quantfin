...
def main():
    ...
    if not sector_df.empty:
      ...
      # Generate Annualised Active Return and Cumulative Active Return Contribution Charts
if 'portfolio' in st.session_state and 'benchmark_df' in st.session_state:
    st.subheader("Annualised Active Return")
    portfolio = st.session_state['portfolio']
    benchmark_df = st.session_state['benchmark_df']

    # Calculate annual returns for portfolio and benchmark
    portfolio_annual_return = portfolio['DailyReturn'].resample('Y').sum() * 100
    benchmark_annual_return = benchmark_df['DailyReturn'].resample('Y').sum() * 100

    # Compute active return
    active_return = portfolio_annual_return - benchmark_annual_return
    active_return_df = pd.DataFrame({
        "Year": active_return.index.year,
        "Active Return": active_return.values
    })

    # Plot annualised active return
    fig_active_return = px.bar(
        active_return_df,
        x="Year",
        y="Active Return",
        title="Annualised Active Return",
        labels={"Active Return": "Active Return (%)", "Year": "Year"},
        color_discrete_sequence=["blue"]
    )
    ...
    for i, stock in enumerate(stocks):
        if stock in portfolio.columns:  # Ensure stock data exists
            stock_returns = portfolio[stock].pct_change().fillna(0)
            stock_contributions[stock] = (stock_returns * (weights[i] / 100)).cumsum()
        else:
            stock_contributions[stock] = pd.Series(0, index=portfolio.index)  # Handle missing stocks

    # Create DataFrame for stock contributions
    stock_contributions_df = pd.DataFrame(stock_contributions, index=portfolio.index)

    # Convert index to string for better visualization in Plotly
    stock_contributions_df.index = stock_contributions_df.index.astype(str)

    # Assign explicit colors for the bar chart
    color_map = px.colors.qualitative.Plotly
    colors = color_map[:len(stocks)]  # Limit to the number of stocks

    # Plot cumulative active return contributions
    ...
    # Render cumulative chart in Streamlit
